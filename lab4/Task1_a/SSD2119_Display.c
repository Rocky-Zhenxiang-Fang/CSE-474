...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    // make PP0-1, PP3-4 outputs
    GPIO_PORTP_DIR_R |= (1 << 0) | (1 << 1) | (1 << 3) | (1 << 4);
    // disable alternate functions
    GPIO_PORTP_AFSEL_R &= ~((1 << 0) | (1 << 1) | (1 << 3) | (1 << 4));  
    // enable digital I/O on PP0-1, PP3-4
    GPIO_PORTP_DEN_R |= (1 << 0) | (1 << 1) | (1 << 3) | (1 << 4);                                            

    // Q0, Q2, Q3
    SYSCTL_RCGCGPIO_R |= (1 << 14);             // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_DIR_R |= (1 << 0) | (1 << 2) | (1 << 3);                                            
    // disable alternate functions
    GPIO_PORTQ_AFSEL_R &= ~((1 << 0) | (1 << 2) | (1 << 3));  
    // enable digital I/O on PQ0, PQ2-3
    GPIO_PORTQ_DEN_R |= (1 << 0) | (1 << 2) | (1 << 3);          
}
